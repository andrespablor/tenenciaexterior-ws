...
(function checkCriticalDependencies() {
    const required = [
        ...
        { name: 'fetchPriceFromFinnhub', module: 'api.js' },
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (settingsBtn) {
        settingsBtn.addEventListener('click', () => {
            ...
            finnhubApiKeyInput.value = appSettings.finnhubApiKey || '';
    ...
    if (saveSettingsBtn) {
        saveSettingsBtn.addEventListener('click', async () => {
            ...
            appSettings.finnhubApiKey = finnhubApiKeyInput.value.trim();
...
async function addToWatchlist() {
    ...
    if (!symbols.length) {
        showToast('⚠️ Símbolos vacíos o ya en la lista', 'warning');
        return;
    }
    ...
    if (addedCount > 0) {
        ...
        input.value = ''; // Clean input only on success
    }
}
...
function deleteSelectedWatchlist() {
    ...
    const toDelete = Array.from(checkboxes).map(cb => cb.dataset.symbol);
    const list = getCurrentWatchlist();
    toDelete.forEach(s => {
    ...
    saveData();
    document.getElementById('watchlist-select-all').checked = false;
    renderWatchlist();
}
...
function updateWatchlistDeleteBtn() {
    ...
    document.getElementById('delete-watchlist-selected').style.display = checked > 0 ? 'inline-flex' : 'none';
}
